...
fn test_navigation_with_hjkl() {
    ...
    editor.state_mut().set_mode(services_editor_vi::EditorMode::Insert);
...
fn test_escape_cancels_command_mode() {
    ...
    // Enter command mode
    editor.process_input(press_key_shift(KeyCode::Semicolon)).unwrap();
    assert_eq!(editor.state().mode(), services_editor_vi::EditorMode::Command);
    ...
    assert_eq!(editor.state().mode(), services_editor_vi::EditorMode::Normal);
