...
//!
//! This module provides a simple built-in font for text rendering.
//! The font is based on the classic VGA 8x16 character set.
...
///
/// Returns a 16-byte array where each byte represents one row of the character.
/// Each bit in the byte represents one pixel (1 = foreground, 0 = background).
...
///
/// This is a minimal font set covering basic ASCII characters.
...
static FONT_DATA: [[u8; 16]; 128] = [
    ...
    [0x00, 0x00, 0x7C, 0xC6, 0xC6, 0xCE, 0xD6, 0xD6,
     0xD6, 0xE6, 0xC6, 0x7C, 0x00, 0x00, 0x00, 0x00], // 0
    ...
    [0x00, 0x00, 0xC6, 0xC6, 0xC6, 0xC6, 0xD6, 0xD6,
     0xD6, 0xFE, 0xEE, 0x6C, 0x00, 0x00, 0x00, 0x00], // W
    [0x00, 0x00, 0xC6, 0xC6, 0x6C, 0x7C, 0x38, 0x38,
     0x7C, 0x6C, 0xC6, 0xC6, 0x00, 0x00, 0x00, 0x00], // X
    ...
    [0x00, 0x00, 0x00, 0x00, 0x00, 0xEC, 0xFE, 0xD6,
     0xD6, 0xD6, 0xD6, 0xC6, 0x00, 0x00, 0x00, 0x00], // m
    ...
    [0x00, 0x00, 0x00, 0x00, 0x00, 0xC6, 0xC6, 0xC6,
     0xC6, 0xC6, 0x6C, 0x38, 0x00, 0x00, 0x00, 0x00], // v
...
mod tests {
    ...
    #[test]
    fn test_get_char_bitmap_valid() {
        let space = get_char_bitmap(b' ');
        assert_eq!(space.len(), 16);
        assert_eq!(space, &[0x00; 16]);
    }

    #[test]
    fn test_get_char_bitmap_a() {
        let a = get_char_bitmap(b'A');
        assert_eq!(a.len(), 16);
        // Just check it's not all zeros (actual font data)
        assert!(a.iter().any(|&b| b != 0));
    }

    #[test]
    fn test_get_char_bitmap_invalid() {
        let invalid = get_char_bitmap(0xFF);
        // Should return '?' character
        assert_eq!(invalid, get_char_bitmap(b'?'));
    }
