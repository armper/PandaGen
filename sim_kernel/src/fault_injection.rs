...
mod tests {
    ...
    fn test_fault_injector_drop_on_channel() {
        ...
        let plan = FaultPlan::new()
            .with_message_fault(MessageFault::DropNextOnChannel { 
                channel: channel1, 
                count: 1 
            });
        let mut injector = FaultInjector::new(plan);
    ...
    fn test_fault_injector_drop_matching() {
        let plan = FaultPlan::new()
            .with_message_fault(MessageFault::DropMatching { 
    ...
    fn test_fault_injector_delay() {
        ...
        let plan = FaultPlan::new()
            .with_message_fault(MessageFault::Delay { duration });
        let mut injector = FaultInjector::new(plan);
    ...
    fn test_fault_injector_reorder() {
        let plan = FaultPlan::new()
            .with_message_fault(MessageFault::ReorderWindow { index: 0, offset: 1 });
    ...
    fn test_fault_injector_crash_on_send() {
        let plan = FaultPlan::new()
            .with_lifecycle_fault(LifecycleFault::CrashOnSend);
        let mut injector = FaultInjector::new(plan);
    ...
    fn test_fault_injector_crash_after_messages() {
        let plan = FaultPlan::new()
            .with_lifecycle_fault(LifecycleFault::CrashAfterMessages { count: 2 });
        let mut injector = FaultInjector::new(plan);
